...
namespace SunEngine
{
	...
	bool SceneRenderer::RenderFrame(CommandBuffer* cmdBuffer, RenderTarget* pOpaqueTarget, RenderTargetPassInfo* pOutputInfo, DeferredRenderTargetPassInfo* pDeferredInfo, RenderTargetPassInfo* pMSAAResolveInfo)
	{
		...
		if (pMSAAResolveInfo)
		{
			...
			pShader = pMSAAResolveInfo->pPipeline->GetShader();
			pShader->Bind(cmdBuffer);
			pMSAAResolveInfo->pPipeline->Bind(cmdBuffer);
			pMSAAResolveInfo->pBindings->Bind(cmdBuffer);

			//draw 2 triangles that are generated by screen vertex shader
			cmdBuffer->Draw(6, 1, 0, 0);

			pMSAAResolveInfo->pBindings->Unbind(cmdBuffer);
			pMSAAResolveInfo->pPipeline->Unbind(cmdBuffer);
			pShader->Unbind(cmdBuffer);
		}
		else
		{
