...
//#define LOAD_SHADER_THREADED 
	...
	bool ShaderMgr::LoadShaders(String& errMsg)
	{
		...
		threadData.defines.push_back(StrFormat("MAX_SKINNED_BONES %d", EngineInfo::GetRenderer().SkinnedBoneMatrices()));
		threadData.defines.push_back(StrFormat("MAX_TEXTURE_TRANSFORMS %d", 1)); //TODO: not sure if supporting this anymore
		...
		threadData.defines.push_back(StrFormat("MAX_TERRAIN_TEXTURES %d", EngineInfo::GetRenderer().TerrainTextures()));
		threadData.defines.push_back(StrFormat("MAX_TERRAIN_SPLAT_MAPS %d", EngineInfo::GetRenderer().TerrainTextures() / 4));
