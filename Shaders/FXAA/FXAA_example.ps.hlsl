#define COLOR(coord) mScene.Sample(mSamplerScene, coord)
#define LUMA(col) sqrt(dot(col.rgb, float3(0.299, 0.587, 0.114)));
#define ONE float4(1,1,1,1)
#define MIN_LUMA 0.0312f
#define MAX_THRESHOLD 0.125f
#define ITERATIONS 12
#define SUBPIXEL_QUALITY 0.75f;

Texture2D screenTexture : register(t0);
Texture2D mDepth : register(t1);

SamplerState mSamplerScene : register(s0);
SamplerState mSamplerDepth : register(s1);
...
static float xOff = 1.0f / 900.0f;
static float yOff = 1.0f / 600.0f;
...
#define texture(tex, uv) tex.Sample(mSamplerScene, uv)
#define textureOffset(tex, uv, offset) tex.Sample(mSamplerScene, uv, offset)
...
#define QUALITY(i) gQuality[i]

float rgb2luma(vec3 rgb){
    return sqrt(dot(rgb, vec3(0.299, 0.587, 0.114)));
}

static float2 inverseScreenSize = float2(1.0f / 900.0f, 1.0f / 600.0f);
